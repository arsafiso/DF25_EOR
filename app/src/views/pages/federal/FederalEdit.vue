...
const formTitle = computed(() => (isEditing.value ? 'Editar Classificação' : 'Criar Classificação Federal'));
...
const validateForm = () => {
    ...
    if (!nome.value || nome.value.trim() === '') {
        errors.nome = 'O nome da Classificação Federal é obrigatório';
        ...
        if (isEditing.value) {
            ...
            if (response.status === 200) {
                toast.add({
                    severity: 'success',
                    summary: 'Sucesso',
                    detail: 'Classificação Federal atualizada com sucesso',
                    life: 3000
                });
                router.push('/federal');
            } else {
            ...
            if (response.status === 201 || response.status === 200) {
                toast.add({
                    severity: 'success',
                    summary: 'Sucesso',
                    detail: 'Classificação Federal criada com sucesso',
                    life: 3000
                });
                router.push('/federal');
            } else {
...
const breadcrumbItems = ref([
    ...
    { label: isEditing.value ? 'Editar Classificação' : 'Criar Classificação Federal', url: null }
]);
...
const saveUser = async () => {
    ...
    try {
        ...
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Usuário adicionado à Classificação Federal', life: 3000 });
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Usuário removido da Classificação Federal', life: 3000 });
            } catch (e) {

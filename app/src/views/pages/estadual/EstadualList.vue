...
function deleteCompany(id: number) {
  confirm.require({
    message: `Você tem certeza que deseja excluir esta Classificação Estadual?`,
    ...
    accept: async () => {
      try {
        ...
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Classificação Estadual removida', life: 3000 });
        fetchCompanies();
      } catch (e) {
...
<template>
  <div class="companies-list p-4">
    <h1>Lista de Classificações Estaduais</h1>
    <Card class="mb-4">
      <template #content>
        <Toolbar class="mb-4">
          <template #start>
